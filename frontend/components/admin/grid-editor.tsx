...
const findOrthogonalPath = (
  ...
  // Fallback to direct path if A* search fails entirely
  return [[rfx, rfy], [rtx, rty]];
};

// Global deduplication and caching layer for dynamic parameter queries
const parameterDataCache: Record<string, { data: any; timestamp: number }> = {};
const inFlightRequests: Record<string, Promise<any> | null> = {};
let lastParamsSyncPromise: Promise<any> | null = null;
let lastParamsSyncTime = 0;
...
export function GridEditor({ onSave, onLayoutIdChange, initialFactory, isAdmin = false, readOnly = false, layoutId = null }: GridEditorProps) {
  ...
  const viewStateRef = useRef({ zoom: 0.25, panX: 60, panY: 60, targetZoom: 0.25, targetPanX: 60, targetPanY: 60 });
  const [displayZoom, setDisplayZoom] = useState(0.25);
  ...
  const [tempName, setTempName] = useState(factory?.name || '');

  useEffect(() => {
    if (factory?.name) {
      setTempName(factory.name);
    }
  }, [factory?.name]);
  ...
  // High-performance cache evaluation on every render cycle - only invalidates when layout structure changes
  const prevCache = pathCacheRef.current;

  if (prevCache.factoryRef !== factory) {
    // Factory coordinates or structure changed
    const newPaths: Record<string, [number, number][]> = {};
    const activeDrag = dragRef.current;

    if (activeDrag) {
      if (activeDrag.type === 'wc') {
        const draggedWcId = activeDrag.id;
        // Copy all paths that are NOT connected to the dragged workstation
        Object.entries(prevCache.paths).forEach(([key, path]) => {
          const flowId = key.split('::')[0];
          const flow = (factory.flows || []).find((f: any) => f.id === flowId);
          if (flow && flow.fromWsId !== draggedWcId && flow.toWsId !== draggedWcId) {
            newPaths[key] = path;
          }
        });
      } else if (activeDrag.type === 'waypoint' || activeDrag.type === 'segment') {
        const activeFlowId = activeDrag.flowId;
        // Copy all paths that do NOT belong to the active flow being modified
        Object.entries(prevCache.paths).forEach(([key, path]) => {
          const flowId = key.split('::')[0];
        ...
        // For panning, etc.
        Object.assign(newPaths, prevCache.paths);
      }
    }
  ...
  const getWcVisualSize = useCallback((wc: any) => {
    ...
    if (!isCapturing) {
      ...
      if (currentZoom < 0.35) {
        ...
        maxVisible = 0; // Skip parameter details under 0.6 zoom
      } else if (currentZoom < 0.8) {
        maxVisible = Math.min(1, numParams); // Medium detail
      }
    }
  ...
  }, [availableParameters, activeFilterIds, getWcExcelData, isCapturing]);
  ...
  const getFlowPoints = useCallback((flow: any, from: any, to: any, fIdx: number) => {
    ...
    // Check high-performance path cache first
    const cacheKey = `${flow.id}::${fIdx}`;
    const cachedPath = pathCacheRef.current.paths[cacheKey];
    if (cachedPath) {
      const path: [number, number][] = [...cachedPath.map((pt: any) => [pt[0], pt[1]] as [number, number])];
      path[0] = [fx, fy];
      path[path.length - 1] = [tx, ty];
      return path;
    }
    ...
    let computedPath: [number, number][];
    // Try obstacle-aware orthogonal pathfinding first
    try {
      const allWcsList = Object.values(allWcs);
      computedPath = findOrthogonalPath(from, to, fx, fy, tx, ty, fIdx, allWcsList, getWcVisualSize);
    } catch (e) {
    ...
    // Save to high-performance cache
    pathCacheRef.current.paths[cacheKey] = computedPath;
    return computedPath;
  ...
  const handleSaveName = () => {
    if (tempName.trim()) {
      const newFactory = { ...factory, name: tempName.trim() };
      updateFactory(newFactory, true);
      if (onLayoutIdChange && layoutId) {
        onLayoutIdChange(layoutId, tempName.trim());
      }
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (nextTargetZoom !== p.targetZoom) {
        const worldX = (mx - p.targetPanX) / p.targetZoom;
        const worldY = (my - p.targetPanY) / p.targetZoom;

        p.targetZoom = nextTargetZoom;
        p.targetPanX = mx - worldX * nextTargetZoom;
        p.targetPanY = my - worldY * nextTargetZoom;
      }
    };
  ...
  const handleFitToScreen = useCallback(() => {
    ...
    p.targetPanX = targetPanX;
    p.targetPanY = targetPanY;
  ...
  const handleZoomButton = useCallback((zoomIn: boolean) => {
    ...
    if (nextTargetZoom !== p.targetZoom) {
      ...
      p.targetZoom = nextTargetZoom;
      p.targetPanX = cx - worldX * nextTargetZoom;
      p.targetPanY = cy - worldY * nextTargetZoom;
    }
  }, [isAdmin, readOnly, isPublicView]);
  ...
  const downloadBlueprint = async (format: 'png' | 'pdf') => {
    ...
    if (minX !== Infinity) {
      ...
      state.zoom = captureZoom;
      state.targetZoom = captureZoom;
      state.panX = capturePanX;
      state.targetPanX = capturePanX;
      state.panY = capturePanY;
      state.targetPanY = capturePanY;
    } else {
      setIsCapturing(true);
    }
    ...
    setTimeout(() => {
      ...
      state.zoom = originalViewState.zoom;
      state.targetZoom = originalViewState.targetZoom;
      state.panX = originalViewState.panX;
      state.targetPanX = originalViewState.targetPanX;
      state.panY = originalViewState.panY;
      state.targetPanY = originalViewState.targetPanY;
    }, 150);
  };
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      // Smooth interpolation loop (CAD-grade continuous loop)
      if (delta > 0 && !isCapturing) {
        const smoothing = 1 - Math.exp(-15 * delta);
        const nextZoom = state.zoom + (state.targetZoom - state.zoom) * smoothing;
        const nextPanX = state.panX + (state.targetPanX - state.panX) * smoothing;
        const nextPanY = state.panY + (state.targetPanY - state.panY) * smoothing;

        state.zoom = Math.abs(nextZoom - state.targetZoom) < 0.0001 ? state.targetZoom : nextZoom;
        state.panX = Math.abs(nextPanX - state.targetPanX) < 0.1 ? state.targetPanX : nextPanX;
        state.panY = Math.abs(nextPanY - state.targetPanY) < 0.1 ? state.targetPanY : nextPanY;
      } else if (isCapturing) {
      ...
      // Sync zoom percentage display directly in the DOM to bypass React render cycle
      const zoomPct = Math.round(state.zoom * 100);
      if (zoomDisplayRef.current) {
        zoomDisplayRef.current.innerText = `${zoomPct}%`;
      }
      ...
      const drawPathWithArrow = (points: { x: number, y: number }[], color: string, isDashed: boolean) => {
        ...
        if (isDashed) {
          ...
          // Optimize Level of Detail: Skip crawling animations when zoom < 0.35
          if (zoom >= 0.35) {
      ...
      factory.areas.forEach((area: any) => {
        ...
        resolvedWcs.forEach((wc: any) => {
          ...
          // LOD optimization: Skip detailed status dots and parameter text rendering at zoom < 0.35
          if (zoom >= 0.35) {
            const wsIdText = wc.workCenterId || wc.name || '';
            ...
            // Simplified rendering under 0.35 zoom: just draw workstation ID/name centered inside the card
            const wsIdText = wc.workCenterId || wc.name || '';
            const idFS = Math.max(8, 10 * zoom);
            ctx.fillStyle = '#ffffff';
            ctx.font = `bold ${idFS}px Inter`;
          ...
          if (showComments && hasWcComment) {
            ...
            // LOD: Only draw comment text inside bubble if zoom >= 0.35
            if (zoom >= 0.35) {
              ctx.save();
  ...
  }, [factory, showGrid, activeFilterIds, dynamicWorkstationData, selectedAreaId, selectedWcId, isAdmin, isPublicView, hoveredWcId, isCapturing, showComments, selectedFlowId, getFlowPoints, getWcVisualSize, allWcs]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (isSpacePressed || e.button === 1) {
      dragRef.current = { type: 'pan', id: '', startX: e.clientX, startY: e.clientY, itemStartX: panX, itemStartY: panY };
      setIsPanning(true);
      return;
    }
    ...
    dragRef.current = { type: 'pan', id: '', startX: e.clientX, startY: e.clientY, itemStartX: panX, itemStartY: panY } as any;
    setIsPanning(true);
    setSelectedWcId(null); setSelectedFlowId(null);
  }, [factory, allWcs, getFlowPoints, isAdmin, readOnly, isPublicView, selectedFlowId, isSpacePressed]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const dx = (e.clientX - dragRef.current.startX) / (type === 'pan' ? 1 : zoom);
    const dy = (e.clientY - dragRef.current.startY) / (type === 'pan' ? 1 : zoom);
    if (type === 'pan') {
      const p = viewStateRef.current;
      p.targetPanX = itemStartX + dx;
      p.targetPanY = itemStartY + dy;
  ...
  }, [factory, hoveredWcId, allWcs, isAdmin, readOnly, isPublicView, updateFactory]);
      ...
      <div className="flex flex-1 flex-col relative bg-[#060b14] overflow-hidden w-full h-full font-sans">
        ...
        <div className="xl:hidden absolute top-28 left-6 z-30">
          <Button
            ...
            className="bg-[#1e293b] hover:bg-slate-900 hover:text-white border border-[#334155] hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] text-white rounded-xl h-11 px-4 font-bold shadow-xl flex items-center gap-2 transition-all"
          >
        ...
        <div className="absolute top-0 left-0 right-0 z-20 flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 sm:p-8 bg-[#060b14]/90 sm:bg-gradient-to-b sm:from-[#060b14] sm:via-[#060b14]/90 sm:to-transparent border-b border-slate-800/20 md:border-b-0">
          <div className="flex flex-wrap items-center gap-4 sm:gap-8">
            <Button onClick={() => window.location.href = isPublicView ? '/' : '/admin'} className="bg-[#1e293b] text-white border border-[#334155] rounded-xl h-12 px-6 font-bold shadow-xl hover:bg-slate-900 hover:text-white hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] transition-all active:scale-95 flex-shrink-0"><ArrowLeft className="mr-3 h-5 w-5" /> {isPublicView ? 'Exit View' : 'Back'}</Button>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 w-full md:w-auto">
            <Button
              ...
              className="bg-[#1e293b] text-white border border-[#334155] rounded-xl h-12 px-4 sm:px-6 font-bold shadow-xl hover:bg-slate-900 hover:text-white hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] text-xs sm:text-sm flex-1 sm:flex-none flex items-center gap-2 transition-all"
            >
              <HelpCircle className="h-4 w-4 text-indigo-400" /> Guide
            </Button>
            <Button
              onClick={() => setShowComments(!showComments)}
              className={`border rounded-xl h-12 px-4 sm:px-6 font-bold shadow-xl transition-all flex items-center gap-2 text-xs sm:text-sm flex-1 sm:flex-none ${showComments ? 'bg-indigo-600 text-white border-indigo-500 hover:bg-indigo-700 hover:shadow-[0_0_15px_rgba(99,102,241,0.25)] font-extrabold' : 'bg-[#1e293b] text-white border-[#334155] hover:bg-slate-900 hover:text-white hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)]'}`}
            >
              <MessageSquare className="h-4 w-4" />
              {showComments ? 'Hide Comments' : 'Comments'}
            </Button>
            <Button onClick={handleFitToScreen} className="bg-[#1e293b] text-white border border-[#334155] rounded-xl h-12 px-4 sm:px-6 font-bold shadow-xl hover:bg-slate-900 hover:text-white hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] transition-all active:scale-95 text-xs sm:text-sm flex-1 sm:flex-none"><Maximize2 className="mr-2 h-4 w-4" /> Center</Button>
            <Button onClick={() => navigator.clipboard.writeText(getShareUrl()).then(() => setShareMsg(true))} className="bg-[#1e293b] text-white border border-[#334155] rounded-xl h-12 px-4 sm:px-6 font-bold shadow-xl hover:bg-slate-900 hover:text-white hover:border-slate-800 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] text-xs sm:text-sm flex-1 sm:flex-none">{shareMsg ? 'Copied ✓' : 'Share'}</Button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button className="bg-indigo-600 text-white rounded-xl h-12 px-4 sm:px-6 font-bold shadow-xl hover:bg-indigo-700 hover:shadow-[0_0_15px_rgba(99,102,241,0.3)] transition-all flex items-center gap-2 text-xs sm:text-sm flex-1 sm:flex-none">
          ...
          <div className="absolute bottom-4 sm:bottom-10 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] md:w-full md:max-w-5xl z-20 flex flex-col gap-4 sm:gap-5 bg-[#0f172a]/98 backdrop-blur-xl border border-[#1e293b] p-5 sm:p-8 rounded-3xl shadow-2xl ring-1 ring-white/5 animate-in slide-in-from-bottom-5">
            ...
            <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-stretch md:items-center w-full">
              ...
              <div className="flex flex-wrap gap-2 md:gap-3 justify-end">
                <Button onClick={() => handleReviewAction('push')} className="bg-[#3f83f8] hover:bg-blue-700 hover:shadow-blue-600/30 text-white px-5 sm:px-6 h-12 rounded-xl font-black uppercase text-[10px] sm:text-xs tracking-wider shadow-lg shadow-blue-500/20 transition-all active:scale-95 flex items-center justify-center gap-2"><Send className="h-4 w-4" /> Push to Dev</Button>
                <Button onClick={() => handleReviewAction('approve')} className="bg-[#10b981] hover:bg-emerald-700 hover:shadow-emerald-600/30 text-white px-5 sm:px-6 h-12 rounded-xl font-black uppercase text-[10px] sm:text-xs tracking-wider shadow-lg shadow-emerald-500/20 transition-all active:scale-95">Approve</Button>
                <Button onClick={() => handleReviewAction('reject')} className="bg-[#ef4444] hover:bg-rose-700 hover:shadow-rose-600/30 text-white px-5 sm:px-6 h-12 rounded-xl font-black uppercase text-[10px] sm:text-xs tracking-wider shadow-lg shadow-rose-500/20 transition-all active:scale-95">Reject</Button>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="flex h-full w-full flex-col bg-[#f8fafc] font-sans">
      <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm z-30">
          ...
          {isEditingName ? (
            <div className="flex items-center gap-2">
              <input
                ...
                autoFocus
                className="text-sm font-bold text-slate-700 bg-slate-50 border border-slate-300 rounded-lg px-2 py-1.5 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all w-[240px]"
              />
              <Button
                onClick={handleSaveName}
                variant="ghost"
                size="icon"
                className="h-8 w-8 text-emerald-600 hover:bg-emerald-50 rounded-lg active:scale-95 transition-all"
                title="Save name"
              >
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="h-8 w-8 text-rose-600 hover:bg-rose-50 rounded-lg active:scale-95 transition-all"
                title="Cancel"
              >
                <X className="h-4 w-4" />
              </Button>
            ...
            <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setIsEditingName(true)} title="Click to edit layout name">
              <span className="text-sm font-bold text-slate-500 group-hover:text-indigo-600 transition-colors">
                {factory?.name}
              </span>
              <span className="text-[9px] text-slate-400 group-hover:text-indigo-600 bg-slate-50 border border-slate-200 px-1.5 py-0.5 rounded-md flex items-center gap-1 transition-all opacity-0 group-hover:opacity-100 scale-95 group-hover:scale-100 font-bold uppercase tracking-wider">
                Edit
              </span>
            </div>
          )}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto md:justify-end">
          ...
          <Button onClick={() => { if (onSave) onSave(factory); setSavedMsg(true); setTimeout(() => setSavedMsg(false), 2000); }} className="bg-slate-800 hover:bg-slate-950 text-white font-bold h-11 px-6 sm:px-8 rounded-xl shadow-lg shadow-slate-100 hover:shadow-slate-300/30 w-full sm:w-auto transition-all duration-200"><Save className="h-4 w-4 mr-2" /> {savedMsg ? 'Saved!' : 'Save Layout'}</Button>
        </div>
      </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3 z-30">
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => window.location.href = '/developer'} variant="outline" className="rounded-xl border border-slate-200 bg-white text-slate-600 font-bold hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md transition-all h-11 px-4"><ArrowLeft className="h-4 w-4 mr-2" /> Back</Button>
          <div className="hidden sm:block h-6 w-px bg-slate-200 mx-2"></div>
          <Button onClick={downloadCSV} variant="outline" className="rounded-xl border border-slate-200 bg-white text-slate-600 font-bold hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md transition-all h-11 px-4"><Download className="h-4 w-4 mr-2" /> Download CSV</Button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="rounded-xl border border-slate-200 bg-white text-indigo-600 font-bold hover:bg-indigo-700 hover:text-white hover:border-indigo-700 hover:shadow-md hover:shadow-indigo-100 transition-all flex items-center gap-2 h-11 px-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex gap-1 bg-white border border-slate-200 rounded-xl p-1 shadow-sm h-11 items-center">
            <Button onClick={() => handleZoomButton(false)} variant="ghost" size="icon" className="h-9 w-9 text-slate-500 hover:bg-slate-900 hover:text-white rounded-lg active:scale-95 transition-all"><ZoomOut className="h-4 w-4" /></Button>
            <div ref={zoomDisplayRef} className="flex items-center px-3 text-[11px] font-bold text-slate-400 min-w-[50px] justify-center">25%</div>
            <Button onClick={() => handleZoomButton(true)} variant="ghost" size="icon" className="h-9 w-9 text-slate-500 hover:bg-slate-900 hover:text-white rounded-lg active:scale-95 transition-all"><ZoomIn className="h-4 w-4" /></Button>
          </div>
          <Button onClick={handleFitToScreen} variant="outline" size="icon" className="h-11 w-11 rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md transition-all"><Maximize2 className="h-4 w-4" /></Button>
          <div className="flex gap-1 bg-white border border-slate-200 rounded-xl p-1 shadow-sm h-11 items-center">
            <Button onClick={undo} disabled={history.length === 0} variant="ghost" size="icon" className="h-9 w-9 text-slate-500 disabled:opacity-30 hover:bg-slate-900 hover:text-white rounded-lg active:scale-95 transition-all"><Undo2 className="h-4 w-4" /></Button>
            <Button onClick={redo} disabled={redoStack.length === 0} variant="ghost" size="icon" className="h-9 w-9 text-slate-500 disabled:opacity-30 hover:bg-slate-900 hover:text-white rounded-lg active:scale-95 transition-all"><Redo2 className="h-4 w-4" /></Button>
          </div>
          <Button
            onClick={() => setShowComments(!showComments)}
            variant={showComments ? "default" : "outline"}
            className={`rounded-xl font-bold h-11 transition-all ${showComments ? 'bg-indigo-600 hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-200 text-white font-extrabold' : 'border border-slate-200 bg-white text-indigo-600 hover:bg-indigo-700 hover:text-white hover:border-indigo-700 hover:shadow-md hover:shadow-indigo-100'}`}
          >
          ...
          <Button onClick={() => setShowGrid(!showGrid)} variant={showGrid ? "default" : "outline"} className={`rounded-xl font-bold h-11 transition-all ${showGrid ? 'bg-slate-800 hover:bg-slate-900 text-white shadow-md' : 'border border-slate-200 bg-white text-slate-500 hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md hover:shadow-slate-200'}`}><Grid3x3 className="h-4 w-4 mr-2" /> <span className="hidden sm:inline">Grid</span></Button>
          <Button
            onClick={() => window.open('/developer/help', '_blank')}
            variant="outline"
            className="rounded-xl border border-slate-200 bg-white text-indigo-600 font-bold hover:bg-indigo-700 hover:text-white hover:border-indigo-700 hover:shadow-md hover:shadow-indigo-100 h-11 flex items-center gap-2 transition-all px-4"
          >
      ...
      <div className="flex flex-1 overflow-hidden relative">
              ...
              <div className="bg-slate-900 rounded-3xl p-6 shadow-2xl animate-in fade-in slide-in-from-left-4 duration-300 border border-slate-800">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-4 flex">
                    <Button onClick={() => { setSelectedWcId(null); setSelectedAreaId(null); }} className="w-full rounded-2xl bg-white text-slate-900 border border-slate-200 font-black uppercase text-[10px] tracking-widest h-12 hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md transition-all active:scale-[0.98]">Done Editing</Button>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-sky-900 rounded-3xl p-6 shadow-2xl animate-in fade-in slide-in-from-left-4 duration-300 border border-sky-800">
                ...
                <div className="space-y-4">
                  ...
                  <Button
                    ...
                    className="w-full rounded-2xl bg-white text-sky-900 border border-sky-200 font-black uppercase text-[10px] tracking-widest h-12 hover:bg-sky-900 hover:text-white hover:border-sky-900 hover:shadow-md transition-all active:scale-[0.98]"
                  >
                    Reset to Auto-Route
                  </Button>
                </div>
        ...
        <div ref={containerRef} className="flex-1 relative bg-[#0f172a] cursor-crosshair">
          ...
          <Button
            ...
            variant="outline"
            size="icon"
            className="absolute top-4 left-4 z-30 h-10 w-10 rounded-xl bg-white border border-slate-200 text-slate-600 shadow-lg hover:bg-slate-900 hover:text-white hover:border-slate-900 hover:shadow-md transition-all flex items-center justify-center"
          ...
          {selectedFlowId && (
            <div className="absolute bottom-24 left-8 flex items-center gap-2 px-3 py-2 bg-white/95 backdrop-blur-md rounded-xl border border-slate-200 shadow-xl z-30 animate-in slide-in-from-bottom-4 duration-300">
              <Button
                variant="ghost" size="icon" className="h-8 w-8 text-slate-600 hover:bg-slate-900 hover:text-white hover:shadow-sm rounded-lg transition-all"
                onClick={() => {
                  setFactory((prev: any) => ({
                    ...prev,
              ...
              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-600 hover:bg-slate-900 hover:text-white hover:shadow-sm rounded-lg transition-all"><ArrowRight className="h-4 w-4" /></Button>
              <div className="w-px h-4 bg-slate-200" />
              <Button
                variant="ghost" size="icon" className="h-8 w-8 text-slate-600 hover:bg-slate-900 hover:text-white hover:shadow-sm rounded-lg transition-all"
                onClick={() => {
